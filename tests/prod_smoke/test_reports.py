...
def test_revenue_overview_loads(browser):
    ...
    assert page.page_title_visible(), "Revenue Metrics title not visible"
...
def test_transactions_report_loads(browser):
    ...
    assert page_is_up(browser)
    assert page.page_title_is_visible(), "Transactions Report page title not visible"
...
def test_general_sales_report_loads(browser):
    ...
    assert page_is_up(browser)
    assert page.section_heading_visible("Sales"), "General Sales Report heading not visible"
...
def test_performance_metrics_loads(browser):
    ...
    assert page_is_up(browser)
    assert page.page_title_visible(), "Performance Metrics title not visible"


@allure.title("PSMO-RPT-009 Redemptions report filter modal opens on page load")
@pytest.mark.prod_smoke
def test_redemptions_report_loads(browser):
    ...
    assert page_is_up(browser)
    assert page.modal_is_open(), "Redemptions filter modal did not open"
