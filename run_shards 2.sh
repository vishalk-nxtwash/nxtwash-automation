...
# ── Launch shards in 2 waves (2 Chrome instances at a time on 8GB RAM) ─────
# Wave 1: shards 1 + 2 in parallel, then Wave 2: shards 3 + 4 in parallel.
# This keeps memory pressure low and prevents disk-swapping slowdowns.
cd "$SCRIPT_DIR"

echo "Wave 1: Shards 1 + 2 running in parallel..."
...
wait $PID_1; EC1=$?
wait $PID_2; EC2=$?
echo "Wave 1 done — Shard 1: $([ "$EC1" -eq 0 ] && echo PASS || echo "FAIL (exit $EC1)") | Shard 2: $([ "$EC2" -eq 0 ] && echo PASS || echo "FAIL (exit $EC2)")"
echo ""

echo "Wave 2: Shards 3 + 4 running in parallel..."
...
wait $PID_3; EC3=$?
wait $PID_4; EC4=$?
echo "Wave 2 done — Shard 3: $([ "$EC3" -eq 0 ] && echo PASS || echo "FAIL (exit $EC3)") | Shard 4: $([ "$EC4" -eq 0 ] && echo PASS || echo "FAIL (exit $EC4)")"
echo ""
